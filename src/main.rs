...
use crate::migrate::commands::MigrateCli;
...
impl DjWizardCommands {
    pub async fn execute(&self) -> DjWizardResult<()> {
        return match self {
            ...
            DjWizardCommands::Migrate(cli) => cli.execute().change_context(DjWizardError).await,
        };
    }
    ...
    pub fn cli_command(&self) -> String {
        match self {
            ...
            DjWizardCommands::Migrate(cli) => cli.to_cli_command(),
        }
    }
}

...
impl GoogleAuth {
    ...
    pub async fn login(&self) -> AuthResult<AuthToken> {
        ...
        // Create OAuth2 client WITHOUT client secret (desktop application with PKCE)
        let client = BasicClient::new(
            ClientId::new(AppConfig::GOOGLE_OAUTH_CLIENT_ID.to_string()),
            None, // No client secret needed for desktop applications
            AuthUrl::new("https://accounts.google.com/o/oauth2/v2/auth".to_string())
                .map_err(|_| AuthError)?,
            Some(TokenUrl::new("https://oauth2.googleapis.com/token".to_string())
                .map_err(|_| AuthError)?),
        )
        .set_redirect_uri(RedirectUrl::new(format!("http://localhost:{}/callback", AppConfig::OAUTH_CALLBACK_PORT))
            .map_err(|_| AuthError)?);
        ...
        println!("If browser doesn't open, visit: {}", auth_url.to_string().blue());
        ...
        if !returned_state.is_empty() && returned_state != *csrf_token.secret() {
            println!("Warning: State mismatch - expected: {}, got: {}", csrf_token.secret(), returned_state);
        }
        ...
        // Get user info
        let user_info = self.get_user_info(token_response.access_token().secret()).await?;

        println!("✅ Successfully authenticated as: {}", user_info.email.green());
    ...
    async fn start_callback_server() -> AuthResult<(String, String)> {
        ...
        let server = Server::http(format!("localhost:{}", AppConfig::OAUTH_CALLBACK_PORT))
            .map_err(|_| AuthError)?;

        println!("Waiting for authentication callback on http://localhost:{}/callback", AppConfig::OAUTH_CALLBACK_PORT);
